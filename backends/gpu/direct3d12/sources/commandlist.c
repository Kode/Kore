...
void kore_d3d12_command_list_begin_render_pass(kore_gpu_command_list *list, const kore_gpu_render_pass_parameters *parameters) {
	...
	for (size_t render_target_index = 0; render_target_index < parameters->color_attachments_count; ++render_target_index) {
		...
		COM_CALL(list->d3d12.device->device, CreateRenderTargetView, render_target->d3d12.resource, &desc, render_target_view);
	...
	if (parameters->depth_stencil_attachment.texture != NULL) {
		...
		COM_CALL(list->d3d12.device->device, CreateDepthStencilView, render_target->d3d12.resource, &desc, depth_stencil_view);
	...
	if (parameters->color_attachments_count > 0) {
		if (parameters->depth_stencil_attachment.texture != NULL) {
			COM_CALL(list->d3d12.list, OMSetRenderTargets, (UINT)parameters->color_attachments_count, &render_target_views, TRUE, &depth_stencil_view);
		}
		else {
			COM_CALL(list->d3d12.list, OMSetRenderTargets, (UINT)parameters->color_attachments_count, &render_target_views, TRUE, NULL);
		}
	}
	else if (parameters->depth_stencil_attachment.texture != NULL) {
		COM_CALL(list->d3d12.list, OMSetRenderTargets, 0, NULL, TRUE, &depth_stencil_view);
	}
	...
	if (parameters->color_attachments_count > 0) {
		D3D12_VIEWPORT viewport = {
		    ...
		    0.0f, 1.0f};
		COM_CALL(list->d3d12.list, RSSetViewports, 1, &viewport);
		...
		COM_CALL(list->d3d12.list, RSSetScissorRects, 1, &scissor);
	}
	else if (parameters->depth_stencil_attachment.texture != NULL) {
		D3D12_VIEWPORT viewport = {
	...
	for (size_t render_target_index = 0; render_target_index < parameters->color_attachments_count; ++render_target_index) {
		if (parameters->color_attachments[render_target_index].load_op == KORE_GPU_LOAD_OP_CLEAR) {
			...
			COM_CALL(list->d3d12.list, ClearRenderTargetView, render_target_view, color, 0, NULL);
		}
	}

	if (parameters->depth_stencil_attachment.texture != NULL) {
		if (parameters->depth_stencil_attachment.depth_load_op == KORE_GPU_LOAD_OP_CLEAR) {
			COM_CALL(list->d3d12.list, ClearDepthStencilView, depth_stencil_view, D3D12_CLEAR_FLAG_DEPTH,
	...
	if (list->d3d12.timestamp_query_set != NULL) {
		COM_CALL(list->d3d12.list, EndQuery, list->d3d12.timestamp_query_set->d3d12.query_heap, D3D12_QUERY_TYPE_TIMESTAMP,
...
void kore_d3d12_command_list_set_index_buffer(kore_gpu_command_list *list, kore_gpu_buffer *buffer, kore_gpu_index_format index_format, uint64_t offset) {
	D3D12_GPU_VIRTUAL_ADDRESS address = COM_CALL_VOID(buffer->d3d12.resource, GetGPUVirtualAddress);
	address += offset;
	...
	COM_CALL(list->d3d12.list, IASetIndexBuffer, &view);
...
void kore_d3d12_command_list_set_vertex_buffer(kore_gpu_command_list *list, uint32_t slot, kore_d3d12_buffer *buffer, uint64_t offset, uint64_t size,
	...
	D3D12_GPU_VIRTUAL_ADDRESS address = COM_CALL_VOID(buffer->resource, GetGPUVirtualAddress);

	view.BufferLocation = address + offset;
	view.SizeInBytes    = (UINT)size;
	view.StrideInBytes  = (UINT)stride;

	COM_CALL(list->d3d12.list, IASetVertexBuffers, slot, 1, &view);
...
void kore_d3d12_command_list_set_render_pipeline(kore_gpu_command_list *list, kore_d3d12_render_pipeline *pipeline) {
	...
	COM_CALL(list->d3d12.list, SetPipelineState, pipeline->pipe);
	COM_CALL(list->d3d12.list, SetGraphicsRootSignature, pipeline->root_signature);
}
...
void kore_d3d12_command_list_draw(kore_gpu_command_list *list, uint32_t vertex_count, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance) {
	COM_CALL(list->d3d12.list, IASetPrimitiveTopology, D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	COM_CALL(list->d3d12.list, DrawInstanced, vertex_count, instance_count, first_vertex, first_instance);
}
...
void kore_d3d12_command_list_draw_indexed(kore_gpu_command_list *list, uint32_t index_count, uint32_t instance_count, uint32_t first_index, int32_t base_vertex,
	...
	COM_CALL(list->d3d12.list, IASetPrimitiveTopology, D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	COM_CALL(list->d3d12.list, DrawIndexedInstanced, index_count, instance_count, first_index, base_vertex, first_instance);
}
...
void kore_d3d12_command_list_set_descriptor_table(kore_gpu_command_list *list, uint32_t table_index, kore_d3d12_descriptor_set *set,
	...
	if (set->allocations[set->current_allocation_index].descriptor_count > 0) {
		...
		if (list->d3d12.compute_pipe != NULL || list->d3d12.ray_pipe != NULL) {
			...
			COM_CALL(list->d3d12.list, SetGraphicsRootDescriptorTable, table_index, gpu_descriptor);
		}
	...
	if (set->allocations[set->current_allocation_index].dynamic_descriptor_count > 0) {
		...
		for (uint32_t descriptor_index = 0; descriptor_index < set->allocations[set->current_allocation_index].dynamic_descriptor_count; ++descriptor_index) {
			...
			D3D12_GPU_VIRTUAL_ADDRESS address = COM_CALL_VOID(dynamic_buffers[descriptor_index]->d3d12.resource, GetGPUVirtualAddress);
...
void kore_d3d12_command_list_set_root_constants(kore_gpu_command_list *list, uint32_t table_index, const void *data, size_t data_size) {
	if (list->d3d12.compute_pipe != NULL || list->d3d12.ray_pipe != NULL) {
		COM_CALL(list->d3d12.list, SetComputeRoot32BitConstants, table_index, (UINT)(data_size / 4), data, 0);
	}
	else {
		COM_CALL(list->d3d12.list, SetGraphicsRoot32BitConstants, table_index, (UINT)(data_size / 4), data, 0);
	}
}
...
void kore_d3d12_command_list_copy_buffer_to_buffer(kore_gpu_command_list *list, kore_gpu_buffer *source, uint64_t source_offset, kore_gpu_buffer *destination,
	...
	COM_CALL(list->d3d12.list, CopyBufferRegion, destination->d3d12.resource, destination_offset, source->d3d12.resource, source_offset, size);
...
void kore_d3d12_command_list_copy_buffer_to_texture(kore_gpu_command_list *list, const kore_gpu_image_copy_buffer *source,
	...
	COM_CALL(list->d3d12.list, CopyTextureRegion, &dst, destination->origin_x, destination->origin_y, 0, &src,
...
void kore_d3d12_command_list_copy_texture_to_buffer(kore_gpu_command_list *list, const kore_gpu_image_copy_texture *source,
	...
	COM_CALL(list->d3d12.list, CopyTextureRegion, &dst, 0, 0, 0, &src, &source_box);
...
void kore_d3d12_command_list_copy_texture_to_texture(kore_gpu_command_list *list, const kore_gpu_image_copy_texture *source,
	...
	COM_CALL(list->d3d12.list, CopyTextureRegion, &dst, destination->origin_x, destination->origin_y, destination->origin_z, &src, &source_box);
...
void kore_d3d12_command_list_compute(kore_gpu_command_list *list, uint32_t workgroup_count_x, uint32_t workgroup_count_y, uint32_t workgroup_count_z) {
	COM_CALL(list->d3d12.list, Dispatch, workgroup_count_x, workgroup_count_y, workgroup_count_z);
}
...
void kore_d3d12_command_list_prepare_raytracing_volume(kore_gpu_command_list *list, kore_gpu_raytracing_volume *volume) {
	...
	build_desc.DestAccelerationStructureData                      = COM_CALL_VOID(volume->d3d12.acceleration_structure.d3d12.resource, GetGPUVirtualAddress);
	build_desc.Inputs                                             = inputs;
	build_desc.ScratchAccelerationStructureData                   = COM_CALL_VOID(volume->d3d12.scratch_buffer.d3d12.resource, GetGPUVirtualAddress);

	COM_CALL(list->d3d12.list, BuildRaytracingAccelerationStructure, &build_desc, 0, NULL);
...
void kore_d3d12_command_list_prepare_raytracing_hierarchy(kore_gpu_command_list *list, kore_gpu_raytracing_hierarchy *hierarchy) {
	...
	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC build_desc = {0};
	build_desc.DestAccelerationStructureData                      = COM_CALL_VOID(hierarchy->d3d12.acceleration_structure.d3d12.resource, GetGPUVirtualAddress);
	build_desc.Inputs                                             = inputs;
	build_desc.ScratchAccelerationStructureData                   = COM_CALL_VOID(hierarchy->d3d12.scratch_buffer.d3d12.resource, GetGPUVirtualAddress);
...
void kore_d3d12_command_list_update_raytracing_hierarchy(kore_gpu_command_list *list, kore_matrix4x4 *volume_transforms, uint32_t volumes_count,
	...
	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC build_desc = {0};
	build_desc.Inputs                                             = inputs;
	build_desc.SourceAccelerationStructureData                    = COM_CALL_VOID(hierarchy->d3d12.acceleration_structure.d3d12.resource, GetGPUVirtualAddress);
	build_desc.DestAccelerationStructureData                      = COM_CALL_VOID(hierarchy->d3d12.acceleration_structure.d3d12.resource, GetGPUVirtualAddress);
	build_desc.ScratchAccelerationStructureData                   = COM_CALL_VOID(hierarchy->d3d12.update_scratch_buffer.d3d12.resource, GetGPUVirtualAddress);
...
void kore_d3d12_command_list_trace_rays(kore_gpu_command_list *list, uint32_t width, uint32_t height, uint32_t depth) {
	...
	desc.MissShaderTable.StartAddress =
	    COM_CALL_VOID(list->d3d12.ray_pipe->shader_ids.d3d12.resource, GetGPUVirtualAddress) + D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT;
	desc.MissShaderTable.SizeInBytes = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
	desc.HitGroupTable.StartAddress =
	    COM_CALL_VOID(list->d3d12.ray_pipe->shader_ids.d3d12.resource, GetGPUVirtualAddress) + 2 * D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT;
...
void kore_d3d12_command_list_set_scissor_rect(kore_gpu_command_list *list, uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
	...
	COM_CALL(list->d3d12.list, RSSetScissorRects, 1, &rect);
}
...
void kore_d3d12_command_list_set_blend_constant(kore_gpu_command_list *list, kore_gpu_color color) {
	...
	COM_CALL(list->d3d12.list, OMSetBlendFactor, color_array);
}

void kore_d3d12_command_list_set_stencil_reference(kore_gpu_command_list *list, uint32_t reference) {
	COM_CALL(list->d3d12.list, OMSetStencilRef, reference);
}
...
void kore_d3d12_command_list_set_name(kore_gpu_command_list *list, const char *name) {
	...
	COM_CALL(list->d3d12.list, SetName, wstr);
}
...
void kore_d3d12_command_list_begin_occlusion_query(kore_gpu_command_list *list, uint32_t query_index) {
	...
	COM_CALL(list->d3d12.list, BeginQuery, list->d3d12.occlusion_query_set->d3d12.query_heap, D3D12_QUERY_TYPE_OCCLUSION, query_index);
}

void kore_d3d12_command_list_end_occlusion_query(kore_gpu_command_list *list) {
	COM_CALL(list->d3d12.list, EndQuery, list->d3d12.occlusion_query_set->d3d12.query_heap, D3D12_QUERY_TYPE_OCCLUSION,
...
void kore_d3d12_command_list_resolve_query_set(kore_gpu_command_list *list, kore_gpu_query_set *query_set, uint32_t first_query, uint32_t query_count,
	...
	COM_CALL(list->d3d12.list, ResolveQueryData, query_set->d3d12.query_heap,
...
void kore_d3d12_command_list_draw_indirect(kore_gpu_command_list *list, kore_gpu_buffer *indirect_buffer, uint64_t indirect_offset, uint32_t max_draw_count,
	...
	COM_CALL(list->d3d12.list, ExecuteIndirect, list->d3d12.render_pipe->draw_command_signature, max_draw_count, indirect_buffer->d3d12.resource,
...
void kore_d3d12_command_list_draw_indexed_indirect(kore_gpu_command_list *list, kore_gpu_buffer *indirect_buffer, uint64_t indirect_offset,
	...
	COM_CALL(list->d3d12.list, ExecuteIndirect, list->d3d12.render_pipe->draw_indexed_command_signature, max_draw_count, indirect_buffer->d3d12.resource,
	         indirect_offset, count_buffer != NULL ? count_buffer->d3d12.resource : NULL, count_offset);
}
...
void kore_d3d12_command_list_compute_indirect(kore_gpu_command_list *list, kore_gpu_buffer *indirect_buffer, uint64_t indirect_offset) {
	COM_CALL(list->d3d12.list, ExecuteIndirect, list->d3d12.compute_pipe->compute_command_signature, 1, indirect_buffer->d3d12.resource, indirect_offset, NULL,
	         0);
}
