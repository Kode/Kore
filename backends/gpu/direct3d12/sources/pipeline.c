...
void kore_d3d12_ray_pipeline_init(kore_gpu_device *device, kore_d3d12_ray_pipeline *pipe, const kore_d3d12_ray_pipeline_parameters *parameters,
	...
	kore_microsoft_affirm(COM_CREATE(device->d3d12.device, CreateStateObject, ID3D12StateObject, &pipe->pipe, &desc));
	...
	COM_CALL(pipe->pipe, QueryInterface, &IID_ID3D12StateObjectProperties, &props);
	...
	memcpy(data, COM_CALL(props, GetShaderIdentifier, raygen), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

	wchar_t miss[1024];
	kore_microsoft_convert_string(miss, parameters->miss_shader_name, 1024);
	memcpy(&data[D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT], COM_CALL(props, GetShaderIdentifier, miss), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

	memcpy(&data[D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT * 2], COM_CALL(props, GetShaderIdentifier, L"HitGroup"), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

	kore_gpu_buffer_unlock(&pipe->shader_ids);

	COM_CALL(props, Release);
