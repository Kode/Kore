...
void kore_d3d12_descriptor_set_set_buffer_view_cbv(kore_gpu_device *device, kore_d3d12_descriptor_set *set, kore_gpu_buffer *buffer, uint32_t index) {
	const D3D12_CONSTANT_BUFFER_VIEW_DESC desc = {
	    .BufferLocation = COM_CALL_VOID(buffer->d3d12.resource, GetGPUVirtualAddress),
	    .SizeInBytes    = (UINT)align_pow2((int)buffer->d3d12.size, 256),
	};
	    ...
	    .Buffer =
	        {
	            .FirstElement        = 0,
	            .NumElements         = 1,
	            .StructureByteStride = (UINT)buffer->d3d12.size,
...
void kore_d3d12_descriptor_set_set_bvh_view_srv(kore_gpu_device *device, kore_d3d12_descriptor_set *set, kore_gpu_raytracing_hierarchy *bvh, uint32_t index) {
	...
	const D3D12_SHADER_RESOURCE_VIEW_DESC desc = {
	    ...
	    .RaytracingAccelerationStructure =
	        {
	            .Location = COM_CALL_VOID(bvh->d3d12.acceleration_structure.d3d12.resource, GetGPUVirtualAddress),
	        },
	};
...
static DXGI_FORMAT convert_d3d12_texture_format(uint32_t format) {
	switch (format) {
...
void kore_d3d12_descriptor_set_set_texture_view_srv(kore_gpu_device *device, kore_d3d12_descriptor_set *set, const kore_gpu_texture_view *texture_view,
	...
	const D3D12_SHADER_RESOURCE_VIEW_DESC desc = {
	    ...
	    .Texture2D =
	        {
	            .MostDetailedMip     = texture_view->base_mip_level,
	            .MipLevels           = texture_view->mip_level_count,
...
void kore_d3d12_descriptor_set_set_bindless_texture_views_srv(kore_gpu_device *device, kore_d3d12_descriptor_set *set,
	...
	for (uint32_t index = 0; index < count; ++index) {
		desc.Format                    = convert_d3d12_texture_format(texture_views[index].texture->d3d12.format);
		desc.Texture2D.MostDetailedMip = texture_views[index].base_mip_level;
		desc.Texture2D.MipLevels       = texture_views[index].mip_level_count;

		D3D12_CPU_DESCRIPTOR_HANDLE descriptor_handle;
		COM_CALL_VOID_RET(device->d3d12.descriptor_heap, GetCPUDescriptorHandleForHeapStart, descriptor_handle);
		descriptor_handle.ptr +=
		    (set->allocations[set->current_allocation_index].bindless_descriptor_allocation.offset + index_start + index) * device->d3d12.cbv_srv_uav_increment;

		COM_CALL(device->d3d12.device, CreateShaderResourceView, texture_views[index].texture->d3d12.resource, &desc, descriptor_handle);
	}
}
	    ...
	    .Texture2DArray =
	        {
	            .MostDetailedMip     = texture_view->base_mip_level,
	            .MipLevels           = texture_view->mip_level_count,
	            .FirstArraySlice     = texture_view->base_array_layer,
	            .ArraySize           = texture_view->array_layer_count,
	            .ResourceMinLODClamp = 0.0f,
	        },
	};
...
void kore_d3d12_descriptor_set_set_texture_view_uav(kore_gpu_device *device, kore_d3d12_descriptor_set *set, const kore_gpu_texture_view *texture_view,
                                                    uint32_t index) {
	const D3D12_UNORDERED_ACCESS_VIEW_DESC desc = {
	    .Format        = (DXGI_FORMAT)texture_view->texture->d3d12.format,
	    .ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D,
	    .Texture2D =
	        {
	            .MipSlice   = texture_view->base_mip_level,
	            .PlaneSlice = 0,
	        },
	};
...
void kore_d3d12_descriptor_set_prepare_cbv_buffer(kore_gpu_command_list *list, kore_gpu_buffer *buffer, uint32_t offset, uint32_t size) {
	if (buffer->d3d12.resource_state != D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER && buffer->d3d12.resource_state != D3D12_RESOURCE_STATE_GENERIC_READ) {
		const D3D12_RESOURCE_BARRIER barrier = {
		    ...
		    .Transition =
		        {
		            .pResource   = buffer->d3d12.resource,
		            .Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,
		            .StateBefore = (D3D12_RESOURCE_STATES)buffer->d3d12.resource_state,
		...
		for (uint32_t mip_level = texture_view->base_mip_level; mip_level < texture_view->base_mip_level + texture_view->mip_level_count; ++mip_level) {
			if (texture_view->texture->d3d12.resource_states[kore_d3d12_texture_resource_state_index(texture_view->texture, mip_level, array_layer)] !=
				...
				const D3D12_RESOURCE_BARRIER barrier = {
				    ...
				    .Transition =
				        {
				            .pResource   = texture_view->texture->d3d12.resource,
				            .Subresource = D3D12CalcSubresource(mip_level, array_layer, 0, texture_view->texture->d3d12.mip_level_count,
	...
	if (texture_view->texture->d3d12.resource_states[kore_d3d12_texture_resource_state_index(texture_view->texture, texture_view->base_mip_level, 0)] !=
		...
		D3D12_RESOURCE_BARRIER barrier = {
		    ...
		    .Transition =
		        {
		            .pResource   = texture_view->texture->d3d12.resource,
		            .Subresource = D3D12CalcSubresource(texture_view->base_mip_level, 0, 0, texture_view->texture->d3d12.mip_level_count,
		                                                texture_view->texture->d3d12.depth_or_array_layers),
		            .StateBefore = (D3D12_RESOURCE_STATES)texture_view->texture->d3d12
