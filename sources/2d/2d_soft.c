...
static kore_matrix3x3_t transform;

void kore_g2_init(int screen_width, int screen_height) {
	kore_g1_init(screen_width, screen_height);
	transform = kore_matrix3x3_identity();
}
...
void kore_g2_clear(float r, float g, float b) {
	memset(kore_internal_g1_image, 0, kore_internal_g1_tex_width * kore_internal_g1_h * 4);
}

/*void kore_g2_draw_image(kore_image_t *img, float x, float y) {
    int xi = (int)kore_round(x);
    int yi = (int)kore_round(y);
    ...
    for (int yy = yi; yy < yi + img->height; ++yy) {
        for (int xx = xi; xx < xi + img->width; ++xx) {
            ...
            if (alpha == 0) {
                ...
                kore_g1_set_pixel(xx, yy, rf, gf, bf);
            }
            else {
                float a = alpha / 255.0f;
                uint32_t old = kore_internal_g1_image[(yy - yi) * kore_internal_g1_tex_width + (xx - xi)];
                float oldblue = ((old >> 16) & 0xff) / 255.0f;
                float oldgreen = ((old >> 8) & 0xff) / 255.0f;
                float oldred = (old & 0xff) / 255.0f;

                kore_g1_set_pixel(xx, yy, rf * a + oldred * (1.0f - a), gf * a + oldgreen * (1.0f - a), bf * a + oldblue * (1.0f - a));
            }
        }
    }
}*/

static void draw_pixel_point(kore_image_t *img, int frame_x, int frame_y, float u, float v) {
	// int xi = (int)kore_round(x);
	// int yi = (int)kore_round(y);
	uint32_t *data = (uint32_t *)img->data;

	int image_x = (int)kore_round(u * (img->width - 1));
	int image_y = (int)kore_round(v * (img->height - 1));
	...
	if (alpha == 0) {
		...
		kore_g1_set_pixel(frame_x, frame_y, rf, gf, bf);
	}
	else {
		float a = alpha / 255.0f;
		uint32_t old = kore_internal_g1_image[frame_y * kore_internal_g1_tex_width + frame_x];
		float oldblue = ((old >> 16) & 0xff) / 255.0f;
		float oldgreen = ((old >> 8) & 0xff) / 255.0f;
		float oldred = (old & 0xff) / 255.0f;

		kore_g1_set_pixel(frame_x, frame_y, rf * a + oldred * (1.0f - a), gf * a + oldgreen * (1.0f - a), bf * a + oldblue * (1.0f - a));
	}
}

#if 0
static void draw_pixel_bilinear(kore_image_t *img, int frame_x, int frame_y, float u, float v) {
	// int xi = (int)kore_round(x);
	// int yi = (int)kore_round(y);
	uint32_t *data = (uint32_t *)img->data;

	int image_x0 = (int)kore_floor(u * (img->width - 1));
	int image_y0 = (int)kore_floor(v * (img->height - 1));
	int image_x1 = kore_mini(image_x0 + 1, img->width - 1);
	int image_y1 = kore_mini(image_y0 + 1, img->height - 1);
...
static void draw_pixel(kore_image_t *img, int frame_x, int frame_y, float u, float v) {
...
static int min4(int a, int b, int c, int d) {
	return kore_mini(kore_mini(a, b), kore_mini(c, d));
}

static int max4(int a, int b, int c, int d) {
	return kore_maxi(kore_maxi(a, b), kore_maxi(c, d));
}

static void drawQuad(kore_image_t *img, Point2D_t v0, Point2D_t v1, Point2D_t v2, Point2D_t v3) {
	...
	minX = kore_maxi(minX, 0);
	minY = kore_maxi(minY, 0);
	maxX = kore_mini(maxX, kore_internal_g1_w - 1);
	maxY = kore_mini(maxY, kore_internal_g1_h - 1);
	...
	for (p.y = minY; p.y <= maxY; p.y++) {
		for (p.x = minX; p.x <= maxX; p.x++) {
			...
			if (u >= 0.0f && u <= 1.0f && v >= 0.0f && v <= 1.0f) {
				...
				// kore_g1_set_pixel(p.x, p.y, u, v, 0.0f);
...
void kore_g2_draw_image(kore_image_t *img, float x, float y) {
	...
	_0 = kore_matrix3x3_multiply_vector(&transform, _0);
	_1 = kore_matrix3x3_multiply_vector(&transform, _1);
	_2 = kore_matrix3x3_multiply_vector(&transform, _2);
	_3 = kore_matrix3x3_multiply_vector(&transform, _3);

	Point2D_t v0, v1, v2, v3;
	v0.x = (int)kore_round(_0.x);
	v0.y = (int)kore_round(_0.y);
	v1.x = (int)kore_round(_1.x);
	v1.y = (int)kore_round(_1.y);
	v2.x = (int)kore_round(_2.x);
	v2.y = (int)kore_round(_2.y);
	v3.x = (int)kore_round(_3.x);
	v3.y = (int)kore_round(_3.y);
	drawQuad(img, v0, v1, v2, v3);
}

void kore_g2_set_rotation(float angle, float centerx, float centery) {
	kore_matrix3x3_t translation1 = kore_matrix3x3_translation(centerx, centery);
	kore_matrix3x3_t rotation = kore_matrix3x3_rotation_z(angle);
	kore_matrix3x3_t translation2 = kore_matrix3x3_translation(-centerx, -centery);
	kore_matrix3x3_t transformation1 = kore_matrix3x3_multiply(&translation1, &rotation);
	transform = kore_matrix3x3_multiply(&transformation1, &translation2);
}
